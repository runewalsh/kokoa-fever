...
class Scene_Battle
  ...
  def use_item
    ...
    targets.each {|target| item.repeats.times { 
      ...
      if unit && unit.cancel? && target.result.cancel
...
class << BattleManager
  ...
  def next_command
    begin
      if !actor || !actor.next_command
        ...
        if OrderManager.top_unit.operate && battler.movable? && ! battler.confusion? # 発動
  ...
  def process_escape
    ...
    if success
      ...
      #$game_party.clear_actions ここを消しました
    end
    wait_for_message
    return success
  end
