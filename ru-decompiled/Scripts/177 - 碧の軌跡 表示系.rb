...
class Spriteset_BattleUnit
  ...
  def init_graphic
    if @battler.actor?
      @graphic_name = actor_graphic_name
      @graphic_index = actor_graphic_index
    else
  ...
  #--------------------------------------------------------------------------
  # ○ バトラーが敵エネミーの場合、使用するグラフィック名を取得します。
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  # ○ バトラーが敵エネミーの場合、使用するグラフィック内のインデックスを取得します。
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  # ○ バトラーが味方の場合に使用するグラフィック名を取得します。
  #--------------------------------------------------------------------------
  def actor_graphic_name
    marker = Saba::Kiseki::GRAPHIC_MARKER
    graphic = get_ex_value_with_index(@battler.actor, marker, 0)
    if graphic == nil
      return @battler.character_name
    else
      return graphic
    end
  end
  #--------------------------------------------------------------------------
  # ○ バトラーが味方の場合に使用するグラフィック内のインデックスを取得します。
  #--------------------------------------------------------------------------
  def actor_graphic_index
    marker = Saba::Kiseki::GRAPHIC_MARKER
    index = get_ex_value_with_index(@battler.actor, marker, 1)
    if index == nil
      return @battler.character_index
    else
      return index.to_i
    end
  end
