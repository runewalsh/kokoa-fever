...
repo = path.normpath(path.join(path.dirname(__file__), ".."))
...
if args.tl or not (args.pngs or args.pngZ or args.wavs):
	subprocess.run([path.join(repo, 'VXAceTranslator\\VXAceTranslator.exe'), '-c', game_inout, '-i', path.join(repo, 'dx\\ru-decompiled')])

	ru_root = path.join(repo, 'dx\\ru-root')
...
if args.wavs and (ffmpeg_path := search_executable("ffmpeg.exe", [repo])):
	for old_fn_rel in ["Audio\\BGM\\マスティア街道東.wav", "Audio\\BGM\\通常戦A.wav"]:
