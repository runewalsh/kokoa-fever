#==============================================================================
#  ■スキルメモライズシステム for RGSS3 Ver1.22-β2
#　□author kure、星潟
#
...
#　　アクター毎に個A別設定したい場合は「<メモライズ不要 1>」と書いて下さい
#==============================================================================
$kure_base_script = {} if $kure_base_script == nil
$kure_base_script[:Memorize] = 1
p "スキルメモライズ"
module KURE
  module SkillMemorize
    ...
    BASE_MEMORIZE_CAPACITY = [0,5,0]
    
    #メモライズ容量Lv補正値
    #Lv_MEMORIZE_CAPACITY = [スキルタイプ1,スキルタイプ2,…]
    Lv_MEMORIZE_CAPACITY = [0.3,0.8,0]
...
class Window_SkillList < Window_Selectable
  ...
  # ● アイテム名の描画(エイリアス再定義)
  #--------------------------------------------------------------------------
  alias k_skill_memorize_draw_item_name draw_item_name
  def draw_item_name(item, x, y, enabled = true, width = 172)
    change_color(crisis_color, enabled) if @actor.extra_skills.include?(item) 
    k_skill_memorize_draw_item_name(item, x, y, enabled, width)
...
class Game_Actor < Game_Battler
  ...
  def fix_memorys
    for i in 0..@memory_skills.size - 1
      next if @skills.include?(@memory_skills[i])
      next if @sub_class_skills.include?(@memory_skills[i])
      next if added_skills.include?(@memory_skills[i])
      @memory_skills[i] = nil
    end
  ...
  # ● スキルを記憶する(追加定義)
  #--------------------------------------------------------------------------
  def add_memorize_skill(skill_id,index = nil)
    unless skill_memorize?($data_skills[skill_id])
      if index
